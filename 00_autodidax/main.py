...
# initialize all of our primitives
add_p = Primitive("add")
mul_p = Primitive("mul")
neg_p = Primitive("neg")
sin_p = Primitive("sin")
...
# We represent active interpreters as a stack. The stack is just a simple list,
# and each element is a container with an integer level (corresponding to the
# element's height in the stack), an interpreter type (which we call a
# trace_type), and an optional field for any global data the interpreter needs.
# MainTrace is the interpreter instance.
@contextmanager
...
# --- next up (tutorial order): ConcreteArray, get_aval, bind, find_top_trace,
# --- full_raise, then the EvalTrace / impl rules ---
...
# The logic in full_raise serves to box values in to Tracers for a particular Trace, calling Different
# methods on the Trace based context: Trace.pure is called on non_Tracer constants, and Trace.lift is called
# for values that are already Tracer's from a lower-level interpreter.
